...
等术语，大多都可以理解为某种意义下的映射，只是强调的侧重点不同。因此，本节我们将利用前面学习的**二元关系**来严格定义映射。    
...
设 $f$ 为映射，且 $\operatorname{dom}(f)=A$，我们也常说：    
> **映射 $f$ 定义在集合 $A$ 上。**

这里的"定义在 $A$ 上"，实际上就是指： $A = \operatorname{dom}(f)$。    

💡 **注意**     
定义中的两个条件分别保证了映射最基本的两个性质。

第一个条件 $\operatorname{dom}(f)=A$，意味着：    
> **集合 $A$ 中的每一个元素都必须参与映射。**
