...
> 若元素 $x$ 与元素 $y$ 满足偏序关系 $xRy$，且 $x\neq y$，我们称 $x$ 在偏序关系上**先于** $y$，或称 $y$ 在偏序关系上**后于** $x$。    
> 这里的"先于"只是描述元素在偏序中的相对位置，并不表示时间上的先后，也不一定表示通常意义上的大小。    
...
**定义 2.2.14** 设 $(A,\le)$ 为偏序集， $B\subseteq A$，且 $b\in B$。    
(1) 若不存在元素， $x\in B$ 满足 $x\neq b \text{ 且 } x\le b$，则称 $b$ 为集合 $B$ 关于 $\le$ 的**极小元**（亦称 **≤-极小元**）。    
(2) 若对于任意 $x\in B$，都有 $b\le x$，则称 $b$ 为集合 $B$ 关于 $\le$ 的**最小元**（亦称 **≤-最小元**）。    
...
1.  偏序集中的每个元素对应图中的一个点。
2.  若元素 $x$ 在偏序关系上先于元素 $y$，则通常把 $y$ 画在 $x$ 的上方。
3.  若元素 $x$ 在偏序关系上先于元素 $y$，并且不存在其他元素 $z$，使得 $x$ 在偏序关系上先于 $z$，且 $z$ 又先于 $y$，则在图中用一条边连接 $x$ 与 $y$。
...
它表示 $a\text{ 先于 } b,\qquad a\text{ 先于 } c,\qquad b\text{ 先于 } d,\qquad c\text{ 先于 } d$。    
根据偏序关系的传递性，还可以推出 $a\text{ 先于 } d$，但由于这一关系能够由已有关系推出，因此图中不再单独画出连接 $a$ 与 $d$ 的线段。

📌**后续关联提示**    
哈斯图中的每一条边对应于偏序中的一种特殊关系，称为**覆盖关系**（cover relation），其对应的元素称为**覆盖元**（cover）。覆盖关系在格论（Lattice）、布尔代数（Boolean algebra）以及偏序理论中具有重要作用。本项目将在后续相关章节中对覆盖元和覆盖关系作严格定义，本节仅利用其直观含义帮助读者阅读哈斯图。    
