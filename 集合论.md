...
**例子 2.3.1**    
(1) 设 $A$ 为一个集合，则 $ID_A={(x,x)\mid x\in A}$ 为 $A$ 到 $A$ 的映射，称作 $A$ 上的**恒等映射**（identity mapping）。    
(2) 设 $A,B$ 是集合，$b\in B$，定义 $g=\lbrace(x,b)\mid x\in A\rbrace$，则 $g$ 是 $A$ 到 $B$ 的映射。    
(3) 定义 $g=\lbrace (x,2x)\mid n\in\mathbb N\rbrace$，则 $g$ 是 $\mathbb N$ 到 $\mathbb N$ 的映射。    
(4) 令 $f=\lbrace(n,\dfrac1{n+1})\mid n\in\mathbb N\rbrace$，则 $f$ 是从 $\mathbb N$ 到 $\mathbb Q$ 的映射。    
