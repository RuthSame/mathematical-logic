...
在日常生活中，我们经常遇到各种"序"。例如，实数之间有大小关系，一个人的学历可以按层次排列，文件夹之间可以按照包含关系排列，一个集合也可以按照包含关系进行比较。    
这些关系都有一个共同特点：它们能够反映某种"先后"、"大小"或"包含"意义，但并不一定要求任意两个对象都能够比较。例如，对于两个集合 $A,B$，有时既不是 $A\subseteq B$，也不是 $B\subseteq A$。    
为了刻画这一类关系，我们引入偏序关系。    
...
直觉上来说，我们可以把偏序关系理解为一种**允许有些元素不能比较大小**的序关系。    
例如，在实数上的小于等于关系中，任意两个实数都能够比较大小；而集合之间的包含关系则不同。    
例如 $\lbrace 1\rbrace\not\subseteq\lbrace2\rbrace, \qquad \lbrace2\rbrace\not\subseteq\lbrace1\rbrace$，因此这两个集合互相不能比较。    
偏序正是为了描述这种"部分元素可以比较，部分元素不能比较"的情形。    
偏序中的"偏"可以看做"部分"的意思。偏序并不要求任意两个元素都能够比较；如果一个偏序中任意两个元素都能够比较，则称它为**全序**（或**线序**）。我们将在后面的章节中介绍全序关系。    
