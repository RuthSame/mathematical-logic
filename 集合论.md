...
**说明：**    
任取 $A\in\mathcal P_{\mathrm{fin}}(\mathbb N)$，由于 $A$ 是有穷集，而 $\mathbb N$ 是无穷集，因此一定存在 $n\in\mathbb N\setminus A$。于是令 $A'=A\cup\{n\}$，则 $A\subsetneq A'$ 且 $A'\in\mathcal P_{\mathrm{fin}}(\mathbb N)$。    
...
$R= \left\{ (m,n)\in\mathbb N\times\mathbb N \;\middle|\; \begin{array}{l} m,n\text{ 同为奇数且 }m\le n,\\ \text{或 }m,n\text{ 同为偶数且 }m\le n,\\ \text{或 }m\text{ 为奇数而 }n\text{ 为偶数 } \end{array} \right\}$     
...
```
这里：   
-  $a$ 与 $b$ 互不可比较，因此它**不是线序**，也就不是良序；
- 但是任意非空子集都存在极小元，因此它仍然是**良基偏序**。
